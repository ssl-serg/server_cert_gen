...
openssl ca -in %TMP_PATH%%CRT_SR_NAME%.csr -cert %TMP_PATH%%CRT_CA_NAME%.pem -keyfile %TMP_PATH%%CRT_CA_NAME%.key -out %TMP_PATH%%CRT_SR_NAME%.crt -batch -config conf_ca.config
...
type  %TMP_PATH%%CRT_SR_NAME%.pem %TMP_PATH%%CRT_SR_NAME%.key %TMP_PATH%%CRT_CA_NAME%.pem > %PUB_PATH%%CRT_SR_NAME%_full_chain_with_key.pem
type  %TMP_PATH%%CRT_SR_NAME%.pem %TMP_PATH%%CRT_CA_NAME%.pem > %PUB_PATH%%CRT_SR_NAME%_full_chain_without_key.pem
type  %PUB_PATH%%CRT_SR_NAME%.pem %TMP_PATH%%CRT_CA_NAME%.pem > %PUB_PATH%%CRT_SR_NAME%_roots_only.pem
